...
>[!info] Cross-Origin Resource Sharing (CORS) — механизм, использующий дополнительные HTTP-заголовки, чтобы дать возможность агенту пользователя получать разрешения на доступ к выбранным ресурсам с сервера на источнике (домене), отличном от того, что сайт использует в данный момент. Т.е. CORS нужен только в случае, если ваш бэкенд обслуживает запросы от фронтенда, который находится на другом домене, порту.
...
## TWA
**TWA (Trusted Web App)** — это, можно сказать, улучшенный PWA. По сути, это уже не сохраненная страница, а полноценный «браузер» на движке Chrome, у которого одно предназначение — отображать ваше веб-приложение.

[Статья на Хабре](https://habr.com/ru/companies/selectel/articles/907834/)
...
Ещё пример сервера, слушающего вебхуки:
```python
...
def git_pull() -> bool:
    try:
        ...
        os.environ["GIT_SSH_COMMAND"] = f"ssh -i {PRIVATE_SSH_KEY_PATH} -o IdentitiesOnly=yes"
        repo = Repo(REPO_DIR)
