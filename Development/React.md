...
>- Инструмент для создания проектов и работы с ними(`Vite`)
...
**Модальное окно**
```jsx
// fixed - жёстко привязан к окну браузера, а не к DOM-иерархии
// inset-0 означает top: 0; right: 0; bottom: 0; left: 0;
// т.е. fixed + inset-0 = "занять весь экран"
<div className="fixed inset-0 z-50 flex items-center justify-center">
      {/* Overlay */}
      // фон для размытия. обязательно с прозрачностью
      //  absolute + inset-0 делаю его слоем, а не участником layout
      <div
      ...
      {/* Modal */}
      // relative позволяет управлять z-index предсказуемо
...
>[!info] Имя компонента в React (который является обычной функцией в JS) пишется с большой буквы, чтобы React понимал, что это преобразуется в пользовательский тэг, а тэги с маленькой буквы - это стандартные HTML тэги.
...
#### Хуки
>[!info] Специальные функции, которые позволяют компонентам использовать состояние и другие возможности React. Хуки можно вызывать только в теле компонента (но не внутри JSX).
