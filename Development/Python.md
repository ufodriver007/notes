...
>[!info] **Пакет** - это модули объединённые по смыслу в папку и файл `__init__.py`(это инициализатор, может быть пустым)
...
#### ЗАМЫКАНИЯ
>[!info] Это ВНУТЕННЯЯ функция, которая возвращается из ВНЕШНЕЙ функции и при этом использует чужие переменные(из внешней обл. видимости). Функция помнит в каком контексте она была создана и может его использовать
...
>[!info] Замыкания - это второй шаг к ООП после `namedtuple`. Хранит данные и предоставляет интерфейс для работы с ними.
...
os.rename('test', 'test.new')   # переименовывает файл или директорию
```
