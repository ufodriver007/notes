...
>[!info] `await`  приводит к выполнению следующей за ней корутины и также приостанавливает родительскую корутину до выполнения. При этом передаёт исполнение событийтому циклу(если результат не отдаётся сразу же). В этом случаем можно добавить `asyncio.sleep(0)`
