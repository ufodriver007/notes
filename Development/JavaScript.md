...
Подключение в HTML. Если подключаем в html обязательно указываем, что это модуль
```html
...
(Math.random() * 100).toFixed(0);  // Случайное число от 0 до 100
```
...
>[!info] При использовании такой *опциональной цепочки* в случае **отсутствия** какого-то из свойств не падает рантайм ошибка, а просто проверка не идёт дальше и возвращает `undefined`.
